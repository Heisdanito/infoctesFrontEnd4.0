...
//  Mobile lecturer login — email + password
//
//  POST /mobile_server/auth/lec/authLogin.php
...
// ── Validate ──────────────────────────────────────────────────
if (empty($email)) {
    sendResponse(false, 'Email is required.', [], 422);
}
if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    sendResponse(false, 'Enter a valid email address.', [], 422);
}
if (empty($password)) {
